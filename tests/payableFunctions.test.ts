...
//import PayableContract from "./example-contracts/payable_contract.json";
...
describe("Test payable message", () => {
  it("Should transfer funds from multisig to other contract", async () => {
    let payableContractAddress = externalContracts["payable_contract.contract"].address;
    ...
    const multisigInitialBalance = await api.query.system.account(multisigAddress);
    ...
    expect(multisigBalance.data.free.toNumber()).to.equal(transferAmount + multisigInitialBalance.data.free.toNumber());

    // Payable contract initial balance
    const payableContractInitialBalance = await api.query.system.account(payableContractAddress);
    ...
    const selector = payableContractMessageIndex.getSelectorByLabel("deposit_funds");
    ...
    const payableContractBalance = await api.query.system.account(payableContractAddress);
    //console.log("Payable contract balance after transfer: ", payableContractBalance.data.free.toNumber());
    expect(payableContractBalance.data.free.toNumber()).to.equal(payableContractInitialBalance.data.free.toNumber() + transferAmount);
  });
});
